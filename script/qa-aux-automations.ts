...
const csv = [
  ...
  '23,11:06:00 AM,0:10:00,11:16:00 AM,TRUE,Session ends / walk-out,none,Green,SS7,,,,,,,',
].join('\n');
...
assert(parsed.rows.length === 7, 'parsed 7 rows');
...
assert(Object.keys(auto.automations).length === 4, '3 cue automations + 1 stop automation');
assert(auto.triggers.length === 4, '4 triggers');
assert(auto.triggers.filter((t) => t.trigger === 'onStart').length === 3, '3 onStart triggers');
...
const main = async () => {
  ...
  assert(r1.written === 4 && r1.removedStale === 0, `first push writes 4, removes 0 (got ${JSON.stringify(r1)})`);
  const s1 = await getAutomationSettings(BASE, null);
  assert(Object.keys(s1.automations).length === 5, 'mock now has 4 ours + 1 manual');
  ...
  assert(r2.written === 4 && r2.removedStale === 4, `second push replaces stale (got ${JSON.stringify(r2)})`);
  const s2 = await getAutomationSettings(BASE, null);
  assert(Object.keys(s2.automations).length === 5, 'no duplicates after re-sync');
  assert(s2.triggers.length === 5, '5 triggers total (4 ours + manual)');
